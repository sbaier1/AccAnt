...
public class CameraSource {
     ...
     */
    private static final float ASPECT_RATIO_TOLERANCE = 100f;
    ...
    private int requestedPreviewWidth = 2048;
    private int requestedPreviewHeight = 1024;
    ...
    public static class Builder {
        private ReceiptReader.OnReceiptDetectedCallback callback;
        private CameraSource cameraSource = new CameraSource();

        public Builder(Context context, ReceiptReader.OnReceiptDetectedCallback callback) {
            if (context == null) {
        ...
        public CameraSource build() {
            cameraSource.frameProcessor = cameraSource.new FrameProcessingRunnable(callback);
            return cameraSource;
        }
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        FrameProcessingRunnable(ReceiptReader.OnReceiptDetectedCallback detector) {
            this.callback = detector;
        }
         ...
         * pausing.
         * <p/>
         * If detection takes longer than the time in between new frames from the camera,
         * we ignore incoming frames until a finished callback in the the receipt detector signals
         * that it has finished processing on its working thread.
         */
        @Override
        public void run() {
            ByteBuffer data;
            final FirebaseVisionTextRecognizer onDeviceTextRecognizer = FirebaseVision.getInstance().getOnDeviceTextRecognizer();
            final ReceiptReader receiptReader = new ReceiptReader();
            final AtomicBoolean canProcess = new AtomicBoolean(true);
            ...
            final ReceiptReader.FinishedDetectionCallback finishedCallback = new ReceiptReader.FinishedDetectionCallback() {
                @Override
                public void finished() {
                    canProcess.set(true);
                }
            };
            final ReceiptReader.OnReceiptDetectedCallback stopCallback = new ReceiptReader.OnReceiptDetectedCallback() {
                @Override
                public void receiptDetected(Receipt receipt) {
                    mActive = false;
                }
            };
            final ArrayList<ReceiptReader.OnReceiptDetectedCallback> callbacks = Lists.newArrayList(stopCallback, callback);
            ...
            while (true) {
                ...
                if (canProcess.get()) {
                    try {
                        final FirebaseVisionImage image = FirebaseVisionImage.fromByteBuffer(data, metadata);
                        canProcess.set(false);
                        final Task<FirebaseVisionText> textRecognition = onDeviceTextRecognizer.processImage(image);
                        textRecognition.addOnSuccessListener(new OnSuccessListener<FirebaseVisionText>() {
                            @Override
                            public void onSuccess(FirebaseVisionText firebaseVisionText) {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                                    // TODO refactor
                                    receiptReader.detectReceipt(firebaseVisionText, callbacks, finishedCallback);
                                }
                            }
                        })
